...
n_neurons_length = len(class_names)

optimal_model = []
optimal_model_parameters = []
highest_accuracy = -math.inf


for h_value in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n_value in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for l_value in l_rate:                  #looking or the best parameters w.r.t the learning rate

            #build the model for each combination by calling the function:
            #model = build_model()
            #-->add your Pyhton code here
            model = build_model(h_value, n_value, n_neurons_length,l_value)
            ...
            current_accuracy = correct_amount / len(class_predicted)
            if(current_accuracy > highest_accuracy):
                highest_accuracy = current_accuracy;
                optimal_model.append(model)
                optimal_model_parameters.append([h_value,n_value,l_value])
                
                print("\nHighest accuracy so far: " + str(highest_accuracy))
                print("--Parameters--\n" + "\nHidden Layers: " + str(h_value) + "\nNeurons: " + str(n_value) + "\nLearning Rate: " + str(l_value))
                print()
...
print("Paramters for optimal model: ",  optimal_model_parameters[-1])
...
model = optimal_model_parameters[-1]
